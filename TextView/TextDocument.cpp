//
//	MODULE:		TextDocument.cpp
//
//	PURPOSE:	Basic implementation of a text data-sequence class
//
//	NOTES:		www.catch22.net
//

#define STRICT
#define WIN32_LEAN_AND_MEAN

#include <windows.h>
#ifdef	_WINDOWS_
#include <usp10.h>
#endif

#include "TextDocument.h"
#include "TextView.h"
#include "Unicode.h"

#include "../mylib/countword.h"
//#include "../convert/HelperBuffer.h"

BOM BOMLOOKUP[] = 
{
	// define longest headers first
	{ 0x0000FEFF, 4, NCP_UTF32    },
	{ 0xFFFE0000, 4, NCP_UTF32BE  },
	{ 0xBFBBEF,	  3, NCP_UTF8	  },
	{ 0xFFFE,	  2, NCP_UTF16BE  },
	{ 0xFEFF,	  2, NCP_UTF16    },
	{ 0,          0, NCP_ASCII	  },
};


int my_strcmp(const WCHAR* str1, int len1, const WCHAR* str2, int len2)
{
    int i;
	int end = min(len1, len2);
    for(i = 0; str1[i] == str2[i]; i++)
    {
        if(i == end - 1)
            return 0;
    }
    return str1[i] - str2[i];
}
//
//	TextDocument constructor
//
TextDocument::TextDocument()
	:m_MuuDllWrapper(false)
{
//	buffer			= 0;
	
	m_nDocLength_bytes  = 0;
	m_nDocLength_chars  = 0;

	m_pLineBuf_byte		= 0;
	m_pLineBuf_char		= 0;
	m_nNumLines			= 0;

	m_nFileFormat		= NCP_UTF8;
	m_nHeaderSize		= 0;

	m_nModifyLineIndex	= 0;
	m_nModifyLineChange	= 0;

	lstrcpy( m_docTitle,	L"Untitled");
	lstrcpy( m_docPath,		L"Untitled.txt");
	EmptyDoc();
}

//
//	TextDocument destructor
//
TextDocument::~TextDocument()
{
	clear();
}
bool TextDocument::save(TCHAR *filename, int ConvertEncodingType, int utf_type)
{
	HANDLE hFile;
	
	hFile = CreateFile(filename, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);

	lstrcpy(m_docPath, filename);
	m_docPath[lstrlen(filename)] = '\0';
	_wsplitpath(filename, NULL, NULL, m_docTitle, NULL);
	
	if(hFile == INVALID_HANDLE_VALUE)
		return false;

	if(ConvertEncodingType == -1)
		return save(hFile);
	else 
		return save(hFile, ConvertEncodingType, utf_type); 
}

bool TextDocument::save(HANDLE hFile, int ConvertEncodingType, int utf_type)
{

	ULONG	written_bytes = 0;
	//write BOM
	{
		DWORD		bom			= 0;
		BYTE*		pBom		= (BYTE*)&bom;
		int			bom_length	= 0;
		ULONG		bom_length_written	= 0;
		fillheader(utf_type, &bom, &bom_length);
		WriteFile(hFile, pBom , bom_length , &bom_length_written, NULL);
		
	}

	// write all lines
	{
		WCHAR*	line_buffer		= new WCHAR[MAX_LINE_LENGTH];
		WCHAR*	convert_buffer	= new WCHAR[MAX_LINE_LENGTH * 2];
		int		line_count	= linecount();
		ULONG	written_bytes_file = 0;
		for(int i = 0; i < line_count ; i++)
		{
			

			ULONG	conv_len_b	= 0; //bytes
			ULONG	conv_len_c	= 0; //chars

			ULONG	written_bytes_step = 0;
			{
				ULONG	line_len_b	= 0; //bytes
				ULONG	line_len_c	= 0; //chars
				ULONG	line_off_b	= 0; //bytes
				ULONG	line_off_c	= 0; //chars

				line_len_c = getline(i, line_buffer, MAX_LINE_LENGTH, &line_off_c, true);
				line_len_b = line_len_c * sizeof(WCHAR);

				switch(ConvertEncodingType)
				{
		/*		case CONVERT_MENK2UNICODE:
					conv_len_c = StandardizeMenkSoft  (line_buffer, line_len_c, convert_buffer, MAX_LINE_LENGTH * 2);
				case CONVERT_FANGZHENG2UNICODE:
					conv_len_c = StandardizeFangZheng ((BYTE*)line_buffer, line_len_b, convert_buffer, MAX_LINE_LENGTH * 2,NULL);*/
				}
				conv_len_b = sizeof(WCHAR) * conv_len_c;
			}

			if(utf_type == NCP_UTF16)
				WriteFile(hFile, convert_buffer, conv_len_b, &written_bytes_step, NULL);
			else if(utf_type == NCP_UTF8)
			{
				WriteFileConvertUtf(hFile, (WCHAR*)convert_buffer, conv_len_c, NCP_UTF8, &written_bytes_step);
			}
			written_bytes_file += written_bytes_step;
		}
		delete[] line_buffer;
		delete[] convert_buffer;
	}
	FlushFileBuffers(hFile);
	CloseHandle(hFile);

	return true;
}
bool TextDocument::WriteFileConvertUtf(HANDLE hFile, WCHAR* buffer16, int length, int dest_format, __out ULONG *written_bytes)
{
	if(written_bytes == NULL) return false;

	bool result = false;
	*written_bytes = 0;
	if(dest_format == NCP_UTF8)
	{
		
		
		int		offset		= 0;	//bytes
		int		STEP		= 256;	//bytes

		int		remain		= length;
		BYTE*	buffer8		= new BYTE[STEP * 4];
		BYTE*   buffer_src	= (BYTE*)buffer16;
		while(offset < length)
		{
			
			size_t	length8				= 1024;
			ULONG	len_bytes_written	= 0;
			int step = min(STEP, remain);
			int tmplen = Convert2Utf8(NCP_UTF16, (BYTE*)(buffer16 + offset), step , buffer8, &length8);
			
			WriteFile(hFile, buffer8 , length8, &len_bytes_written, NULL);
			offset += tmplen;
			remain -= tmplen;
			*written_bytes += len_bytes_written;
		}
		delete []buffer8;
	}
	return true;
}

bool TextDocument::save(HANDLE hFile)
{
	DWORD	bom			= 0;
	BYTE*	pBom		= (BYTE*)&bom;
	int		bom_length	= 0;
	BYTE*	buffer16	= new BYTE[m_nDocLength_bytes];
	//BYTE*	buffer_		= new BYTE[O_LENGTH];

	ULONG	len_bytes_written = 0;
	fillheader(m_nFileFormat, &bom, &bom_length);
	WriteFile(hFile, pBom , bom_length		, &len_bytes_written, NULL);
	

	m_seq.render(0, buffer16, m_nDocLength_bytes);
	if(m_nFileFormat == NCP_UTF16)
		WriteFile(hFile, buffer16	, m_nDocLength_bytes, &len_bytes_written, NULL);
	else if(m_nFileFormat == NCP_UTF8)
	{
		WriteFileConvertUtf(hFile, (WCHAR*)buffer16, charCount(), NCP_UTF8, &len_bytes_written);
	}
	FlushFileBuffers(hFile);
	CloseHandle(hFile);
	delete[] buffer16;
	return true;
}

//
//	Initialize the TextDocument with the specified file
//
bool TextDocument::init(TCHAR *filename, int ConvertEncodingType)
{
	HANDLE hFile;

	lstrcpy(m_docPath, filename);
	m_docPath[lstrlen(filename)] = '\0';
	_wsplitpath(filename, NULL, NULL, m_docTitle, NULL);

	hFile = CreateFile(filename, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);

	if(hFile == INVALID_HANDLE_VALUE)
		return false;

	if(ConvertEncodingType == -1)
		return init(hFile);
	else
		return init(hFile, ConvertEncodingType);
}
bool TextDocument::init_from_utf16(WCHAR*buffer, ULONG length)
{
	return init((BYTE*)buffer, length * sizeof(WCHAR), NCP_UTF16);
}
bool TextDocument::init(BYTE* pBuffer, long fileLength, int format)
{
	//char*	buffer;
	TCHAR*  pBuffer_utf16	= 0;
	size_t	length_utf16	= 0;
	int		header_size	= 0;

	m_nDocLength_bytes = fileLength;
	if(format == NCP_UNKNOWN)
		m_nFileFormat		= detect_file_format((BYTE*)pBuffer, fileLength, &header_size);
	else 
		m_nFileFormat       = format;
	if(m_nFileFormat == NCP_UTF16)
	{
		pBuffer_utf16	= (TCHAR*)(pBuffer + header_size);
		m_nDocLength_bytes -= header_size;
		m_seq.init((BYTE *)pBuffer_utf16, m_nDocLength_bytes);
	}
	else if(m_nFileFormat == NCP_UTF8)
	{
		length_utf16	= m_nDocLength_bytes * 2;
		pBuffer_utf16	= new TCHAR[length_utf16];
		int result		= file_to_utf16(
								m_nFileFormat , 
								(BYTE*)(pBuffer + header_size) , 
								m_nDocLength_bytes - header_size , 
								pBuffer_utf16 , 
								&length_utf16 
							);
		if(result == 0) 
		{
			delete[] pBuffer_utf16;
			return false;
		}
		m_nDocLength_bytes = length_utf16 * sizeof(TCHAR);
		m_seq.init((BYTE *)pBuffer_utf16, m_nDocLength_bytes);
		delete[] pBuffer_utf16;
	}
	return true;
}
//
//	Initialize using a file-handle
//
bool TextDocument::init(HANDLE hFile)
{
	ULONG	numread;
	BYTE*	buffer;
	TCHAR*  pBuffer_utf16	= 0;
	size_t	length_utf16	= 0;
	int		header_size		= 0;
	//int		file_format		= 0;
	if((m_nDocLength_bytes	= GetFileSize(hFile, 0)) == 0)
		return false;
	
	// allocate new file-buffer
	if((buffer = new BYTE[m_nDocLength_bytes]) == 0)
		return false;

	// read entire file into memory
	ReadFile(hFile, buffer, m_nDocLength_bytes, &numread, 0);

	m_nFileFormat	= detect_file_format((BYTE*)buffer, numread, &header_size);
	
	bool result		= init(buffer + header_size, m_nDocLength_bytes- header_size, m_nFileFormat);
	if(!result) return false;
	//if(m_nFileFormat == NCP_UTF16)
	//{
	//	pBuffer_utf16	= (TCHAR*)(buffer + header_size);
	//	m_nDocLength_bytes -= header_size;
	//	m_seq.init((BYTE *)pBuffer_utf16, m_nDocLength_bytes);
	//}
	//else if(m_nFileFormat == NCP_UTF8)
	//{
	//	length_utf16	= m_nDocLength_bytes * 2;
	//	pBuffer_utf16	= new TCHAR[length_utf16];
	//	int result		= file_to_utf16(
	//							m_nFileFormat , 
	//							(BYTE*)(buffer + header_size) , 
	//							m_nDocLength_bytes - header_size , 
	//							pBuffer_utf16 , 
	//							&length_utf16 
	//						);
	//	if(result == 0) 
	//	{
	//		delete[] buffer;
	//		return false;
	//	}
	//	m_nDocLength_bytes = length_utf16 * sizeof(TCHAR);
	//	m_seq.init((BYTE *)pBuffer_utf16, m_nDocLength_bytes);
	//	delete[] pBuffer_utf16;
	//}

	// try to detect if this is an ascii/unicode/utf8 file
	// m_nFileFormat = detect_file_format(&m_nHeaderSize);
	// work out where each line of text starts
	if(!init_linebuffer())
		clear();

	CloseHandle(hFile);

	delete[] buffer;
	return true;
}
bool TextDocument::init(HANDLE hFile, int ConvertEncodingType)
{
	bool		result		= false;
	LONG		nFileSize	= 0;	
	BYTE	*	buffer		= 0;

	if(!m_MuuDllWrapper.IsReady()) 
		return false;
	CHelperBuffer hBuffer(&m_MuuDllWrapper);

	if((nFileSize	= GetFileSize(hFile, 0)) == 0)
		return false;
	
	// allocate new file-buffer
	if((buffer = new BYTE[nFileSize]) == 0)
		return false;
	
	// read entire file into memory
	{
		ULONG numread = 0;
		ReadFile(hFile, buffer, nFileSize, &numread, 0);
		if(numread != nFileSize) 
			return false;
	}


	{
		LONG file_cursor		= 0;
		/*WCHAR *convert_buffer	= new WCHAR[nFileSize * 2];*/
		//HelperBuffer hcon_buffer= CreateHelperBuffer();
		{

			ULONG file_length	= nFileSize;
			ULONG conv_length	= nFileSize * 2;

			if(ConvertEncodingType == CONVERT_FANGZHENG2UNICODE)
			{
				m_MuuDllWrapper.ConvertMongolian(
					CONVERT_FANGZHENG2UNICODE,
					&buffer[file_cursor],
					&file_length ,
					&hBuffer);
			}
			else if(ConvertEncodingType == CONVERT_MENK2UNICODE)
			{
				TextDocument tmpDoc ;
				
				tmpDoc.clear();
				tmpDoc.init((BYTE*)&buffer[file_cursor], file_length);
				ULONG charCount	= tmpDoc.charCount();
				WCHAR* buffer16 = new WCHAR[charCount];
				tmpDoc.m_seq.render(0, (BYTE*)buffer16, tmpDoc.m_nDocLength_bytes);
				m_MuuDllWrapper.ConvertMongolian(
					CONVERT_MENK2UNICODE,
					buffer16,
					&charCount,
					&hBuffer);

				//StandardizeMenkSoftHB(

				//	buffer16,
				//	&charCount,
				//	hcon_buffer);
				delete[] buffer16;
			}

			m_nDocLength_bytes		= hBuffer.GetBufferSize();
			WCHAR *convert_buffer	= new WCHAR[m_nDocLength_bytes / 2];
			hBuffer.GetBufferDate((BYTE*)convert_buffer, m_nDocLength_bytes);
			{
				CHelperBuffer hCorrect_buffer(&m_MuuDllWrapper);
				ULONG byte_length = m_nDocLength_bytes / sizeof(WCHAR);
				//CorrectMongolWordHB((WCHAR*)convert_buffer, &byte_length, hCorrect_buffer);
				m_MuuDllWrapper.CorrectMongolian((WCHAR*)convert_buffer, &byte_length, &hCorrect_buffer);

				//CorrectMongolWordHB_regex((WCHAR*)convert_buffer, &byte_length, hCorrect_buffer);
				m_nDocLength_bytes = hCorrect_buffer.GetBufferSize();
				WCHAR *correct_buffer	= new WCHAR[m_nDocLength_bytes / 2];

				hCorrect_buffer.GetBufferDate((BYTE*)correct_buffer, m_nDocLength_bytes);
				m_seq.init((BYTE*)correct_buffer, m_nDocLength_bytes);
				delete[] correct_buffer;

			}
			//DestroyHelperBuffer(hcon_buffer);
			delete[] convert_buffer;
			if(!init_linebuffer())
				clear();
			
		}

	}
	
	
	if(buffer)	
		delete[] buffer;
	CloseHandle(hFile);

	return true;
}
	//Initialize the TextDocument with the specified file

/*bool TextDocument::save(TCHAR *filename)
{
	HANDLE hFile;
	
	hFile = CreateFile(filename, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);

	if(hFile == INVALID_HANDLE_VALUE)
		return false;



	CloseHandle(hFile);
	return true;
}*/


//
//	Parse the file lo
//
//
//	From the unicode.org FAQ:
//
//	00 00 FE FF			UTF-32, big-endian 
//	FF FE 00 00			UTF-32, little-endian 
//	FE FF				UTF-16, big-endian 
//	FF FE				UTF-16, little-endian 
//	EF BB BF			UTF-8 
//
//	Match the first x bytes of the file against the
//  Byte-Order-Mark (BOM) lookup table
//
int TextDocument::detect_file_format(const BYTE* buffer,int buffer_length, __out int *header_size)
{
	BYTE header[4]	= { 0 };
	int header_len	= min(4, buffer_length);
	for(int i = 0; i < header_len; i++)
	{
		header[i] = buffer[i];
	}
	for(int i = 0; BOMLOOKUP[i].len; i++)
	{
		if(header_len >= BOMLOOKUP[i].len &&
		   memcmp(header, &BOMLOOKUP[i].bom, BOMLOOKUP[i].len) == 0)
		{
			*header_size = BOMLOOKUP[i].len;
			return BOMLOOKUP[i].type;
		}
	}

	*header_size = 0;
	return NCP_UTF8;	// default to ASCII
}
int TextDocument::detect_file_format(int *m_nHeaderSize)
{
	BYTE header[4] = { 0 };
	m_seq.render(0, header, 4);

	for(int i = 0; BOMLOOKUP[i].len; i++)
	{
		if(m_nDocLength_bytes >= BOMLOOKUP[i].len &&
		   memcmp(header, &BOMLOOKUP[i].bom, BOMLOOKUP[i].len) == 0)
		{
			*m_nHeaderSize = BOMLOOKUP[i].len;
			return BOMLOOKUP[i].type;
		}
	}

	*m_nHeaderSize = 0;
	return NCP_ASCII;	// default to ASCII
}


//
//	Empty the data-TextDocument
//
bool TextDocument::clear()
{
	m_seq.clear();
	m_nDocLength_bytes	= 0;
	m_nHeaderSize		= 0;

	if(m_pLineBuf_byte)
	{
		delete[] m_pLineBuf_byte;
		m_pLineBuf_byte = 0;
	}

	if(m_pLineBuf_char)
	{
		delete[] m_pLineBuf_char;
		m_pLineBuf_char = 0;
	}
		
	m_nNumLines = 0;
	return true;
}

bool TextDocument::EmptyDoc()
{
	clear();
	m_seq.init();

	// this is not robust. it's just to get the thing
	// up-and-running until I write a proper line-buffer mananger
	m_pLineBuf_byte = new ULONG[0x1000];
	m_pLineBuf_char = new ULONG[0x1000];

	m_pLineBuf_byte[0] = 0;
	m_pLineBuf_char[0] = 0;

	return true;
}


//
//	Return a UTF-32 character value
//
int TextDocument::getchar(ULONG offset, ULONG lenbytes, ULONG *pch32)
{
//	BYTE	*rawdata   = (BYTE *)(buffer + offset + m_nHeaderSize);
	BYTE	rawdata[16];

	lenbytes = min(16, lenbytes);
	m_seq.render(offset+ m_nHeaderSize, rawdata, lenbytes);


	UTF16   *rawdata_w = (UTF16 *)rawdata;//(WCHAR*)(buffer + offset + m_nHeaderSize);
	WCHAR     ch16;
	size_t   ch32len = 1;
	return utf16_to_utf32(rawdata_w, lenbytes / 2, pch32, &ch32len) * sizeof(WCHAR);
	//switch(m_nFileFormat)
	//{
	//case NCP_ASCII:
	//	MultiByteToWideChar(CP_ACP, 0, (CCHAR*)rawdata, 1, &ch16, 1);
	//	*pch32 = ch16;
	//	return 1;

	//case NCP_UTF16:
	//	return utf16_to_utf32(rawdata_w, lenbytes / 2, pch32, &ch32len) * sizeof(WCHAR);
	//	
	//case NCP_UTF16BE:
	//	return utf16be_to_utf32(rawdata_w, lenbytes / 2, pch32, &ch32len) * sizeof(WCHAR);

	//case NCP_UTF8:
	//	return utf8_to_utf32(rawdata, lenbytes, pch32);

	//default:
	//	return 0;
	//}

}

//
//	Fetch a buffer of UTF-16 text from the specified byte offset - 
//  returns the number of characters stored in buf
//
//	Depending on how Neatpad was compiled (UNICODE vs ANSI) this function
//  will always return text in the "native" format - i.e. Unicode or Ansi -
//  so the necessary conversions will take place here.
//
//  TODO: make sure the CR/LF is always fetched in one go
//        make sure utf-16 surrogates kept together
//		  make sure that combining chars kept together
//		  make sure that bidirectional text kep together (will be *hard*) 
//
//	offset   - BYTE offset within underlying data sequence
//	lenbytes - max number of bytes to process (i.e. to limit to a line)
//  buf		 - UTF16/ASCII output buffer
//	plen	 - [in] - length of buffer, [out] - number of code-units stored
//
//	returns  - number of bytes processed
//
ULONG TextDocument::gettext(ULONG offset, ULONG lenbytes, TCHAR *buf, ULONG *buflen)
{
//	BYTE	*rawdata = (BYTE *)(buffer + offset + m_nHeaderSize);

	ULONG chars_copied = 0;
	ULONG bytes_processed = 0;

	if(offset >= m_nDocLength_bytes)
	{
		*buflen = 0;
		return 0;
	}

	while(lenbytes > 0 && *buflen > 0)
	{
		BYTE   rawdata[0x100];
		size_t rawlen = min(lenbytes, 0x100);

		// get next block of data from the piece-table
		m_seq.render(offset + m_nHeaderSize, rawdata, rawlen);

		// convert to UTF-16 
		size_t tmplen = *buflen;
		rawlen = rawdata_to_utf16(rawdata, rawlen, buf, &tmplen);

		lenbytes		-= rawlen;
		offset			+= rawlen;
		bytes_processed += rawlen;

		buf				+= tmplen;
		*buflen			-= tmplen;
		chars_copied	+= tmplen;
	}

	*buflen = chars_copied;
	return bytes_processed;

	//ULONG remaining = lenbytes;
	//int   charbuflen = *buflen;

	//while(remaining)
/*	{
		lenbytes = min(lenbytes, sizeof(rawdata));
		m_seq.render(offset + m_nHeaderSize, rawdata, lenbytes);

#ifdef UNICODE

	switch(m_nFileFormat)
	{
	// convert from ANSI->UNICODE
	case NCP_ASCII:
		return ascii_to_utf16(rawdata, lenbytes, buf, (size_t*)buflen);
		
	case NCP_UTF8:
		return utf8_to_utf16(rawdata, lenbytes, buf, (size_t*)buflen);

	// already unicode, do a straight memory copy
	case NCP_UTF16:
		return copy_utf16((WCHAR*)rawdata, lenbytes/sizeof(WCHAR), buf, (size_t*)buflen);

	// need to convert from big-endian to little-endian
	case NCP_UTF16BE:
		return swap_utf16((WCHAR*)rawdata, lenbytes/sizeof(WCHAR), buf, (size_t*)buflen);

	// error! we should *never* reach this point
	default:
		*buflen = 0;
		return 0;	
	}

#else

	switch(m_nFileFormat)
	{
	// we are already an ASCII app, so do a straight memory copy
	case NCP_ASCII:

		int len;
		
		len = min(*buflen, lenbytes);
		memcpy(buf, rawdata, len);

		*buflen = len;
		return len;

	// anything else is an error - we cannot support Unicode or multibyte
	// character sets with a plain ASCII app.
	default:
		*buflen = 0;
		return 0;
	}

#endif

	//	remaining -= lenbytes;
	//	buf       += lenbytes;
	//	offset    += lenbytes;
	}*/
}

ULONG TextDocument::getdata(ULONG offset, BYTE *buf, size_t len)
{
	//memcpy(buf, buffer + offset + m_nHeaderSize, len);
	m_seq.render(offset + m_nHeaderSize, buf, len);
	return len;
}
void TextDocument::update_linebuffer()
{
	for(int i = m_nModifyLineIndex + 1; i<= m_nNumLines; i++)
	{
		m_pLineBuf_char[i] += m_nModifyLineChange;
		m_pLineBuf_byte[i] += (m_nModifyLineChange * sizeof(WCHAR));
	}
	m_nModifyLineChange = 0;
}
//
//	Initialize the line-buffer
//
//	With Unicode a newline sequence is defined as any of the following:
//
//	\u000A | \u000B | \u000C | \u000D | \u0085 | \u2028 | \u2029 | \u000D\u000A
//
bool TextDocument::init_linebuffer()
{
	ULONG offset_bytes		= 0;
	ULONG offset_chars		= 0;
	ULONG linestart_bytes	= 0;
	ULONG linestart_chars	= 0;
	ULONG bytes_left	    = m_nDocLength_bytes - m_nHeaderSize;

	ULONG buflen  = m_nDocLength_bytes - m_nHeaderSize;

	m_nModifyLineChange		= 0;

	delete[] m_pLineBuf_byte;
	delete[] m_pLineBuf_char;
	// allocate the line-buffer for storing each line's BYTE offset
	if((m_pLineBuf_byte = new ULONG[buflen+1]) == 0)
		return false;

	// allocate the line-buffer for storing each line's CHARACTER offset
	if((m_pLineBuf_char = new ULONG[buflen+1]) == 0)
		return false;

	m_nNumLines = 0;


	// loop through every byte in the file
	for(offset_bytes = 0; offset_bytes < buflen; )
	{
		ULONG ch32;

		// get a UTF-32 character from the underlying file format.
		// this needs serious thought. Currently 
		ULONG len = getchar(offset_bytes, buflen - offset_bytes, &ch32);
		offset_bytes += len;
		offset_chars += 1;

		if(ch32 == '\r')
		{
			// record where the line starts
			m_pLineBuf_byte[m_nNumLines]	= linestart_bytes;
			m_pLineBuf_char[m_nNumLines]	= linestart_chars;
			linestart_bytes					= offset_bytes;
			linestart_chars					= offset_chars;

			// look ahead to next char
			len = getchar(offset_bytes, buflen - offset_bytes, &ch32);
			offset_bytes += len;
			offset_chars += 1;

			// carriage-return / line-feed combination
			if(ch32 == '\n')
			{
				linestart_bytes		= offset_bytes;
				linestart_chars		= offset_chars;
			}
			
			m_nNumLines++;
		}
		else if(ch32 == '\n' || ch32 == '\x0b' || ch32 == '\x0c' || ch32 == 0x0085 || ch32 == 0x2029 || ch32 == 0x2028)
		{
			// record where the line starts
			m_pLineBuf_byte[m_nNumLines]	= linestart_bytes;
			m_pLineBuf_char[m_nNumLines]	= linestart_chars;
			linestart_bytes					= offset_bytes;
			linestart_chars					= offset_chars;
			m_nNumLines++;
		}
		// force a 'hard break' 
		else if(offset_chars - linestart_chars >= MAX_LINE_LENGTH)
		{
			m_pLineBuf_byte[m_nNumLines]	= linestart_bytes;
			m_pLineBuf_char[m_nNumLines]	= linestart_chars;
			linestart_bytes					= offset_bytes;
			linestart_chars					= offset_chars;
			m_nNumLines++;
		}
	}

	if(buflen > 0)
	{
		m_pLineBuf_byte[m_nNumLines] = linestart_bytes;
		m_pLineBuf_char[m_nNumLines] = linestart_chars;
		m_nNumLines++;
	}

	m_pLineBuf_byte[m_nNumLines] = buflen;
	m_pLineBuf_char[m_nNumLines] = offset_chars;

	
	return true;
}


//
//	Return the number of lines
//
ULONG TextDocument::linecount()
{
	return m_nNumLines;
}

//
//	Return the length of longest line
//
ULONG TextDocument::longestline(int tabwidth)
{
	//ULONG i;
	ULONG longest = 0;
	ULONG xpos = 0;
//	char *bufptr = (char *)(buffer + m_nHeaderSize);
/*
	for(i = 0; i < length_bytes; i++)
	{
		if(bufptr[i] == '\r')
		{
			if(bufptr[i+1] == '\n')
				 i++;

			longest = max(longest, xpos);
			xpos = 0;
		}
		else if(bufptr[i] == '\n')
		{
			longest = max(longest, xpos);
			xpos = 0;
		}
		else if(bufptr[i] == '\t')
		{
			xpos += tabwidth - (xpos % tabwidth);
		}
		else
		{
			xpos ++;
		}
	}

	longest = max(longest, xpos);*/
	return 100;//longest;
}

//
//	Return information about specified line
//
bool TextDocument::lineinfo_from_lineno(ULONG lineno, ULONG *lineoff_chars,  ULONG *linelen_chars, ULONG *lineoff_bytes, ULONG *linelen_bytes)
{
	if(lineno < m_nNumLines)
	{
		if(lineno < m_nModifyLineIndex)
		{
			if(linelen_chars) *linelen_chars  = m_pLineBuf_char[lineno+1] - m_pLineBuf_char[lineno];
			if(lineoff_chars) *lineoff_chars  = m_pLineBuf_char[lineno];

			if(linelen_bytes) *linelen_bytes  = m_pLineBuf_byte[lineno+1] - m_pLineBuf_byte[lineno];
			if(lineoff_bytes) *lineoff_bytes  = m_pLineBuf_byte[lineno] ;

		}
		else if(m_nModifyLineIndex == lineno)
		{
			if(linelen_chars) *linelen_chars  = m_pLineBuf_char[lineno+1] - m_pLineBuf_char[lineno] + m_nModifyLineChange;
			if(lineoff_chars) *lineoff_chars  = m_pLineBuf_char[lineno];

			if(linelen_bytes) *linelen_bytes  = m_pLineBuf_byte[lineno+1] - m_pLineBuf_byte[lineno] + m_nModifyLineChange *sizeof(WCHAR);
			if(lineoff_bytes) *lineoff_bytes  = m_pLineBuf_byte[lineno] ;
		}
		else
		{
			if(linelen_chars) *linelen_chars  = m_pLineBuf_char[lineno+ 1] - m_pLineBuf_char[lineno] ;
			if(lineoff_chars) *lineoff_chars  = m_pLineBuf_char[lineno] + m_nModifyLineChange;
			if(linelen_bytes) *linelen_bytes  = m_pLineBuf_byte[lineno+1] - m_pLineBuf_byte[lineno] ;
			if(lineoff_bytes) *lineoff_bytes  = m_pLineBuf_byte[lineno] + m_nModifyLineChange *sizeof(WCHAR);

		}
		return true;
	}
	else
	{
		return false;
	}
}

//
//	Perform a reverse lookup - file-offset to line number
//
bool TextDocument::lineinfo_from_offset(ULONG offset_chars, ULONG *lineno, ULONG *lineoff_chars, ULONG *linelen_chars, ULONG *lineoff_bytes, ULONG *linelen_bytes)
{
	ULONG low  = 0;
	ULONG high = m_nNumLines-1;
	ULONG line = 0;

	if(m_nNumLines == 0)
	{
		if(lineno)			*lineno			= 0;
		if(lineoff_chars)	*lineoff_chars	= 0;
		if(linelen_chars)	*linelen_chars  = 0;
		if(lineoff_bytes)	*lineoff_bytes	= 0;
		if(linelen_bytes)	*linelen_bytes  = 0;

		return false;
	}


	int modify_offset = 0 ;
	int modify_length = 0 ;
	while(low <= high)
	{
		line = (high + low) / 2;

		if(line == m_nModifyLineIndex)
		{
			modify_length = m_nModifyLineChange;
		}
		else if(line < m_nModifyLineIndex)
		{
			modify_offset = 0;
		}
		else
		{
			modify_offset = m_nModifyLineChange;
		}

		if(offset_chars >= m_pLineBuf_char[line] + modify_offset && offset_chars < m_pLineBuf_char[line + 1] + modify_length + modify_offset)
		{
			break;
		}
		else if(offset_chars < m_pLineBuf_char[line] + modify_offset)
		{
			high = line-1;
		}
		else
		{
			low = line+1;
		}
	}

	if(lineno)			*lineno			= line;
	if(lineoff_bytes)	*lineoff_bytes	= m_pLineBuf_byte[line] + (modify_offset * sizeof(WCHAR));
	if(linelen_bytes)	*linelen_bytes  = m_pLineBuf_byte[line+1] - m_pLineBuf_byte[line] + (modify_length * sizeof(WCHAR));
	if(lineoff_chars)	*lineoff_chars  = m_pLineBuf_char[line] + modify_offset;
	if(linelen_chars)	*linelen_chars  = m_pLineBuf_char[line+1] - m_pLineBuf_char[line] + modify_length;

	return true;
}

int TextDocument::getformat()
{
	return m_nFileFormat;
}

int TextDocument::setformat(int format)
{
	m_nFileFormat = format;
	return 0;
}

ULONG TextDocument::size_bytes()
{
	return m_nDocLength_bytes;
}
ULONG TextDocument::charCount()
{
	return byteoffset_to_charoffset(m_nDocLength_bytes - m_nHeaderSize);
}
bool TextDocument::getStatistic(STATISTIC *pStat)
{
#ifdef	_WINDOWS_

	if(pStat == NULL) 
		return false;

	bool ret = true;
	ZeroMemory(pStat, sizeof(STATISTIC));
	int	item_buffer_size	= 10;
	int item_count			= 0;
	WCHAR *line_buffer		= new WCHAR[MAX_LINE_LENGTH];
	SCRIPT_ITEM		*pItems = (SCRIPT_ITEM *)malloc(sizeof(SCRIPT_ITEM) * item_buffer_size);
	//SCRIPT_CONTROL	sc	= { 0 };
	//SCRIPT_STATE	ss	= { 0 };
	const SCRIPT_PROPERTIES **propList = NULL;
	int		propCount		= 0;

	// get pointer to the global script table
	ScriptGetProperties(&propList, &propCount);

	for(int line = 0; line < linecount(); line++)
	{
		int char_count	= getline(line, line_buffer, MAX_LINE_LENGTH, NULL);
		HRESULT hr		= S_OK;




		if(char_count == 0) continue;
		while((hr =	ScriptItemize(
			line_buffer, 
			char_count, 
			item_buffer_size,
			NULL,
			NULL,
			pItems,
			&item_count)) == E_OUTOFMEMORY )
		{
			//delete[] pItems;
			//pItems = 0;	
			item_buffer_size += 100;
			pItems = (SCRIPT_ITEM *)realloc(pItems, sizeof(SCRIPT_ITEM) * item_buffer_size);
			if(pItems == 0) 
			{
				ret = false;
				goto FINAL;
			}
		}
		//int langid_mongol = MAKELANGID(LANG_MONGOLIAN, SUBLANG_MONGOLIAN_PRC);
		if(hr == S_OK)
		{
			for(int i = 0; i < item_count; i++)
			{
				int item_char_count = pItems[i+1].iCharPos - pItems[i].iCharPos;
				if(propList[pItems[i].a.eScript]->langid == LANG_MONGOLIAN)
				{
					int count = CountWord(line_buffer, item_char_count, L" ");
					pStat->nMongolWord += count;
				}
				pStat->nMongolChar += item_char_count;
			}
		}
		else
		{
			ret = false;
			goto FINAL;
		}
			

	}
	
FINAL:
	if(line_buffer)
		delete[] line_buffer;
	if(pItems)
		free(pItems);
	return ret;
#else
	return false;
#endif
}
TextIterator TextDocument::iterate(ULONG offset_chars)
{
	ULONG off_bytes = charoffset_to_byteoffset(offset_chars);
	ULONG len_bytes = m_nDocLength_bytes - off_bytes;

	//if(!lineinfo_from_offset(offset_chars, 0, linelen, &offset_bytes, &length_bytes))
	//	return TextIterator();

	return TextIterator(off_bytes, len_bytes, this);
}


//
//
//
TextIterator TextDocument::iterate_line(ULONG lineno, ULONG *linestart, ULONG *linelen)
{
	ULONG offset_bytes;
	ULONG length_bytes;

	if(!lineinfo_from_lineno(lineno, linestart, linelen, &offset_bytes, &length_bytes))
		return TextIterator();
	
	return TextIterator(offset_bytes, length_bytes, this);
}

TextIterator TextDocument::iterate_line_offset(ULONG offset_chars, ULONG *lineno, ULONG *linestart)
{
	ULONG offset_bytes;
	ULONG length_bytes;

	if(!lineinfo_from_offset(offset_chars, lineno, linestart, 0, &offset_bytes, &length_bytes))
		return TextIterator();
	
	return TextIterator(offset_bytes, length_bytes, this);
}

ULONG TextDocument::lineno_from_offset(ULONG offset)
{
	ULONG lineno = 0;
	lineinfo_from_offset(offset, &lineno, 0, 0, 0, 0);
	return lineno;
}

ULONG TextDocument::offset_from_lineno(ULONG lineno)
{
	ULONG lineoff = 0;
	lineinfo_from_lineno(lineno, &lineoff, 0, 0, 0);
	return lineoff;
}

//
//	Retrieve an entire line of text
//	
ULONG TextDocument::getline(ULONG nLineNo, TCHAR *buf, ULONG buflen, ULONG *off_chars, bool fWithLineBreaker)
{
	ULONG offset_bytes;
	ULONG length_bytes;
	ULONG offset_chars;
	ULONG length_chars;

	if(!lineinfo_from_lineno(nLineNo, &offset_chars, &length_chars, &offset_bytes, &length_bytes))
	{
		*off_chars = 0;	
		return 0;
	}

	gettext(offset_bytes, length_bytes, buf, &buflen);
	
	if(off_chars) *off_chars = offset_chars;

	if(!fWithLineBreaker)
		buflen -= CRLF_size(buf, buflen);
	return buflen;
}
int TextDocument::CRLF_size(TCHAR *szText, int nLength)
{
	if(nLength >= 2)
	{
		if(szText[nLength-2] == '\r' && szText[nLength-1] == '\n') 
			return 2;
	}

	if(nLength >= 1)
	{
		if( szText[nLength-1] == '\r'	|| szText[nLength-1] == '\n' || 
			szText[nLength-1] == '\x0b' || szText[nLength-1] == '\x0c' ||
			szText[nLength-1] == '\x85' || szText[nLength-1] == 0x2028 || 
			szText[nLength-1] == 0x2029)
			return 1;
	}	

	return 0;
}
//int	TextDocument::Convert2Utf8(
//								   __in  int		format, 
//								   __in  BYTE*		buffer, 
//								   __in  size_t		buffer_length, 
//								   __out BYTE *		utf8str, 
//								   __out size_t *	utf8len
//							   )
//{
//	switch(format)
//	{
//	// convert from ANSI->UNICODE
//	case NCP_ASCII:
//		return 0;
//		
//	case NCP_UTF8:
//		return 0;//utf8_to_utf16(buffer, file_length, (UTF16 *)utf16str, utf16len);
//
//	// already unicode, do a straight memory copy
//	case NCP_UTF16:
//		return utf16_to_utf8((UTF16 *)buffer, buffer_length, utf8str,  utf8len);
//		//return 0;
//	//	rawlen /= sizeof(TCHAR);
//	//	return copy_utf16((UTF16 *)rawdata, rawlen, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);
//
//	// need to convert from big-endian to little-endian
//	//case NCP_UTF16BE:
//	//	buffer_length /= sizeof(TCHAR);
//	//	return swap_utf16((UTF16 *)buffer, buffer_length, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);
//
//	// error! we should *never* reach this point
//	default:
//		*utf8len = 0;
//		return 0;	
//	}
//}
int TextDocument::file_to_utf16(
								   __in  int		file_format, 
								   __in  BYTE   *	buffer, 
								   __in  size_t		file_length, 
								   __out TCHAR  *	utf16str, 
								   __out size_t *	utf16len
								)
{
	switch(file_format)
	{
	// convert from ANSI->UNICODE
	case NCP_ASCII:
		return ascii_to_utf16(buffer, file_length, (UTF16 *)utf16str, utf16len);
		
	case NCP_UTF8:
		return utf8_to_utf16(buffer, file_length, (UTF16 *)utf16str, utf16len);

	// already unicode, do a straight memory copy
	case NCP_UTF16:
		return 0;
	//	rawlen /= sizeof(TCHAR);
	//	return copy_utf16((UTF16 *)rawdata, rawlen, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);

	// need to convert from big-endian to little-endian
	case NCP_UTF16BE:
		file_length /= sizeof(TCHAR);
		return swap_utf16((UTF16 *)buffer, file_length, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);

	// error! we should *never* reach this point
	default:
		*utf16len = 0;
		return 0;	
	}
}
//
//	Convert the RAW buffer in underlying file-format to UTF-16
//
//	
//	utf16len	- [in/out]	on input holds size of utf16str buffer, 
//							on output holds number of utf16 characters stored
//
//	returns bytes processed from rawdata
//
size_t TextDocument::rawdata_to_utf16(const BYTE *rawdata, size_t rawlen, TCHAR *utf16str, size_t *utf16len)
{
	//switch(m_nFileFormat)
	//{
	//// convert from ANSI->UNICODE
	//case NCP_ASCII:
	//	return ascii_to_utf16(rawdata, rawlen, (UTF16 *)utf16str, utf16len);
	//	
	//case NCP_UTF8:
	//	return utf8_to_utf16(rawdata, rawlen, (UTF16 *)utf16str, utf16len);

	//// already unicode, do a straight memory copy
	//case NCP_UTF16:
		rawlen /= sizeof(TCHAR);
		return copy_utf16((UTF16 *)rawdata, rawlen, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);

	// need to convert from big-endian to little-endian
	//case NCP_UTF16BE:
	//	rawlen /= sizeof(TCHAR);
	//	return swap_utf16((UTF16 *)rawdata, rawlen, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);

	//// error! we should *never* reach this point
	//default:
	//	*utf16len = 0;
	//	return 0;	
	//}
}

//
//	Converts specified UTF16 string to the underlying RAW format of the text-document
//	(i.e. UTF-16 -> UTF-8
//		  UTF-16 -> UTF-32 etc)
//
//	returns number of WCHARs processed from utf16str
//
size_t TextDocument::utf16_to_rawdata(const TCHAR *utf16str, size_t utf16len, BYTE *rawdata, size_t *rawlen)
{
	//switch(m_nFileFormat)
	//{
	//// convert from UTF16 -> ASCII
	//case NCP_ASCII:
	//	return utf16_to_ascii((UTF16 *)utf16str, utf16len, rawdata, rawlen);
		
	// convert from UTF16 -> UTF8
	//case NCP_UTF8:
	//	return utf16_to_utf8((UTF16 *)utf16str, utf16len, rawdata, rawlen);

	// already unicode, do a straight memory copy
	//case NCP_UTF16:
		*rawlen /= sizeof(TCHAR);
		utf16len = copy_utf16((UTF16 *)utf16str, utf16len, (UTF16 *)rawdata, rawlen);
		*rawlen *= sizeof(TCHAR);
		return utf16len;

	// need to convert from big-endian to little-endian
	//case NCP_UTF16BE:
	//	*rawlen /= sizeof(TCHAR);
	//	utf16len = swap_utf16((UTF16 *)utf16str, utf16len, (UTF16 *)rawdata, rawlen);
	//	*rawlen *= sizeof(TCHAR);
	//	return utf16len;

	//// error! we should *never* reach this point
	//default:
	//	*rawlen = 0;
	//	return 0;	
	//}

}

//
//	Insert UTF-16 text at specified BYTE offset
//
//	returns number of BYTEs stored
//
ULONG TextDocument::insert_raw(ULONG offset_bytes, const TCHAR *text, ULONG length)
{
	BYTE  buf[0x100];
	size_t buflen;
	ULONG copied;
	ULONG rawlen = 0;
	ULONG offset = offset_bytes + m_nHeaderSize;
	while(length)
	{
		buflen = 0x100;
		copied = utf16_to_rawdata(text, length, buf, (size_t *)&buflen);

		// do the piece-table insertion!
		if(!m_seq.insert(offset, buf, buflen))
			break;

		text   += copied;
		length -= copied;
		rawlen += buflen;
		offset += buflen;
	}
	m_nDocLength_bytes = m_seq.size();
	return rawlen;
}

ULONG TextDocument::replace_raw(ULONG offset_bytes, const TCHAR *text, ULONG length, ULONG erase_chars)
{
	BYTE  buf[0x100];
	ULONG buflen;
	ULONG copied;
	ULONG rawlen = 0;
	ULONG offset = offset_bytes + m_nHeaderSize;

	ULONG erase_bytes	= count_chars(offset_bytes, erase_chars);
	while(length)
	{
		buflen = 0x100;
		copied = utf16_to_rawdata(text, length, buf, (size_t *)&buflen);

		// do the piece-table replacement!
		if(!m_seq.replace(offset, buf, buflen, erase_bytes))
			break;

		text   += copied;
		length -= copied;
		rawlen += buflen;
		offset += buflen;

		erase_bytes = 0;
	}

	m_nDocLength_bytes = m_seq.size();
	return rawlen;
}

//
//	Erase is a little different. Need to work out how many
//  bytes the specified number of UTF16 characters takes up
//
ULONG TextDocument::erase_raw(ULONG offset_bytes, ULONG length)
{
	/*TCHAR  buf[0x100];
	ULONG  buflen;
	ULONG  bytes;
	ULONG  erase_bytes  = 0;
	ULONG  erase_offset = offset_bytes;

	while(length)
	{
		buflen = min(0x100, length);
		bytes = gettext(offset_bytes, 0x100, buf, &buflen); 

		erase_bytes  += bytes;
		offset_bytes += bytes;
		length       -= buflen;
	}

	// do the piece-table deletion!
	if(m_seq.erase(erase_offset + m_nHeaderSize, erase_bytes))
	{
		m_nDocLength_bytes = m_seq.size();
		return length;
	}*/

	ULONG erase_bytes  = count_chars(offset_bytes, length);
	
	if(m_seq.erase(offset_bytes + m_nHeaderSize, erase_bytes))
	{
		m_nDocLength_bytes = m_seq.size();
		return length;
	}
		
	return 0;
}

//
//	return number of bytes comprising 'length_chars' characters
//	in the underlying raw file
//
ULONG TextDocument::count_chars(ULONG offset_bytes, ULONG length_chars)
{
	return length_chars * sizeof(WCHAR);
	//switch(m_nFileFormat)
	//{
	//case NCP_ASCII:
	//	return length_chars;

	//case NCP_UTF16:
	//case NCP_UTF16BE:
	//	return length_chars * sizeof(WCHAR);

	//default:
	//	break;
	//}

	//ULONG offset_start = offset_bytes;

	//while(length_chars && offset_bytes < m_nDocLength_bytes)
	//{
	//	TCHAR buf[0x100];
	//	ULONG charlen	= min(length_chars, 0x100);
	//	ULONG bytelen;

	//	bytelen			= gettext(offset_bytes, m_nDocLength_bytes - offset_bytes, buf, &charlen);

	//	length_chars	-= charlen;
	//	offset_bytes	+= bytelen;
	//}

	//return offset_bytes - offset_start;
}

ULONG TextDocument::byteoffset_to_charoffset(ULONG offset_bytes)
{
	//switch(m_nFileFormat)
	//{
	//case NCP_ASCII:
	//	return offset_bytes;

	//case NCP_UTF16:
	//case NCP_UTF16BE:
		return offset_bytes / sizeof(WCHAR);

	//case NCP_UTF8:
	//case NCP_UTF32:
	//case NCP_UTF32BE:
	//	// bug bug! need to implement this. 
	//	{
	//		break;
	//	}
	//default:
	//	break;
	//}

	//return 0;
}

ULONG TextDocument::charoffset_to_byteoffset(ULONG offset_chars)
{
	//switch(m_nFileFormat)
	//{
	//case NCP_ASCII:
	//	return offset_chars;

	//case NCP_UTF16:
	//case NCP_UTF16BE:
		return offset_chars * sizeof(WCHAR);

	//case NCP_UTF8:
	//case NCP_UTF32:
	//case NCP_UTF32BE:
	//default:
	//	break;
	//}

	//ULONG lineoff_chars;
	//ULONG lineoff_bytes;

	//if(lineinfo_from_offset(offset_chars, 0, &lineoff_chars, 0, &lineoff_bytes, 0))
	//{
	//	return count_chars(lineoff_bytes, offset_chars - lineoff_chars) 
	//			+ lineoff_bytes;
	//}
	//else
	//{
	//	return 0;
	//}
}

//
//	Insert text at specified character-offset
//
ULONG TextDocument::insert_text(ULONG offset_chars, const TCHAR *text, ULONG length)
{
	//ULONG length_bytes	= m_nDocLength_bytes;

	bool fUpdate = false;

	if(m_nModifyLineIndex >= 0 && 
		m_nModifyLineIndex < m_nNumLines && 
		(offset_chars < m_pLineBuf_char[m_nModifyLineIndex] || offset_chars >= m_pLineBuf_char[m_nModifyLineIndex + 1] + m_nModifyLineChange))
	{
		update_linebuffer();
		
		fUpdate = true;
		
	}

	ULONG offset_bytes	= charoffset_to_byteoffset(offset_chars) ;
	int ret = insert_raw(offset_bytes, text, length);

	lineinfo_from_offset(offset_chars, (ULONG*)&m_nModifyLineIndex, 0, 0, 0, 0);
	m_nModifyLineChange++;

	if(m_nNumLines == 0)
	{
		init_linebuffer();
		fUpdate = true;
	}
	if(fUpdate)
	{
		
	}
	
	return ret;
}
ULONG TextDocument::replace_text(const TCHAR *find_text, ULONG find_length, const TCHAR *text, ULONG length)
{
	ULONG found		= -1;
	ULONG offset	= 0;
	WCHAR buf[MAX_LINE_LENGTH];
	m_seq.group();
	while(find(offset, find_text, find_length, &found))
	{
		
		m_seq.replace_group_no_change(found * sizeof(TCHAR), (seqchar*)text ,length * sizeof(TCHAR), find_length * sizeof(TCHAR));
		m_nDocLength_bytes = m_seq.size();
		//m_seq.render(0, (seqchar*)buf, m_nDocLength_bytes);
		offset = found + length;
	}
	m_seq.ungroup();
	
	return 0;
}
//
//	Overwrite text at specified character-offset
//
ULONG TextDocument::replace_text(ULONG offset_chars, const TCHAR *text, ULONG length, ULONG erase_len)
{
	ULONG offset_bytes = charoffset_to_byteoffset(offset_chars);
	return replace_raw(offset_bytes, text, length, erase_len);
}

//
//	Erase text at specified character-offset
//
ULONG TextDocument::erase_text(ULONG offset_chars, ULONG length)
{
	ULONG offset_bytes = charoffset_to_byteoffset(offset_chars);
	return erase_raw(offset_bytes, length);
}

bool TextDocument::Undo(ULONG *offset_start, ULONG *offset_end)
{
	ULONG start, length;

	if(!m_seq.undo())
		return false;

	start				= m_seq.event_index() - m_nHeaderSize;
	length				= m_seq.event_length();

	*offset_start		= byteoffset_to_charoffset(start);
	*offset_end			= byteoffset_to_charoffset(start + length);

	m_nDocLength_bytes	= m_seq.size();
	
	return true;
}

bool TextDocument::Redo(ULONG *offset_start, ULONG *offset_end)
{
	ULONG start, length;

	if(!m_seq.redo())
		return false;

	start				= m_seq.event_index() - m_nHeaderSize;
	length				= m_seq.event_length();

	*offset_start		= byteoffset_to_charoffset(start);
	*offset_end			= byteoffset_to_charoffset(start+length);
	
	m_nDocLength_bytes	= m_seq.size();

	return true;
}

bool TextDocument::fillheader(int format, DWORD* pHeader, int * pHeader_length)
{
	if(pHeader == 0 || pHeader_length == 0) return false;
	int count = sizeof(BOMLOOKUP) / sizeof(BOM);

	for(int i = 0; i < count; i++)
	{
		if(BOMLOOKUP[i].type == format)
		{
			*pHeader		= BOMLOOKUP[i].bom;
			*pHeader_length = BOMLOOKUP[i].len;
			return true;
		}
	}
	return false;

}
void TextDocument::getfilename(__out WCHAR* bufFullPath, __out WCHAR* bufTitle)
{
	if(bufFullPath	)lstrcpy(bufFullPath	, m_docPath );
	if(bufTitle		)lstrcpy(bufTitle		, m_docTitle);
}
bool TextDocument::find(ULONG start_offset_chars, const TCHAR *text, ULONG length, ULONG *find_offset)
{
	//ULONG byte_offset = charoffset_to_byteoffset(start_offset_chars);
	//ULONG byte_length = m_nDocLength_bytes - byte_offset;
	//WCHAR buffer[MAX_LINE_LENGTH];
	//gettext(byte_offset, byte_length,buffer, MAX_LINE_LENGTH);
	
	for(Iterator itor	= GetIterator(start_offset_chars);
		*itor != '\0' ;
		++ itor )
	{
		if(strcmp(text, length, itor) == 0)
		{
			*find_offset = itor.Offset();
			return true;
		}
	}

	return false;
}
int	TextDocument::strcmp(const TCHAR *text, ULONG length, Iterator itor)
{
	int dif = 0;
	int i	= 0;
	for(i	= 0; 
		i < length && (dif = text[i] - *itor) == 0; 
		++itor, ++i);

	return dif;
}

LONG TextDocument::GetConvertLookupTable(convert_type* convert_looup)
{

	if(m_MuuDllWrapper.IsReady()) 
	{
		return m_MuuDllWrapper.GetConvertLookup((mdw_convert_type*)convert_looup);
	}
	else
		return -1;
}
int	TextDocument::Convert2Utf8(
								   __in  int		format, 
								   __in  BYTE*		buffer, 
								   __in  size_t		buffer_length, 
								   __out BYTE *		utf8str, 
								   __out size_t *	utf8len
							   )
{
	switch(format)
	{
	// convert from ANSI->UNICODE
	case NCP_ASCII:
		return 0;
		
	case NCP_UTF8:
		return 0;//utf8_to_utf16(buffer, file_length, (UTF16 *)utf16str, utf16len);

	// already unicode, do a straight memory copy
	case NCP_UTF16:
		return utf16_to_utf8((UTF16 *)buffer, buffer_length, utf8str,  utf8len);
		//return 0;
	//	rawlen /= sizeof(TCHAR);
	//	return copy_utf16((UTF16 *)rawdata, rawlen, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);

	// need to convert from big-endian to little-endian
	//case NCP_UTF16BE:
	//	buffer_length /= sizeof(TCHAR);
	//	return swap_utf16((UTF16 *)buffer, buffer_length, (UTF16 *)utf16str, utf16len) * sizeof(TCHAR);

	// error! we should *never* reach this point
	default:
		*utf8len = 0;
		return 0;	
	}
}